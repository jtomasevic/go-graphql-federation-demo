...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		switch typeName {
		case "Actor":
			resolverName, err := entityResolverNameForActor(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Actor": %w`, err)
			}
			switch resolverName {

			case "findActorByImdbID":
				id0, err := ec.unmarshalNString2string(ctx, rep["imdbId"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findActorByImdbID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindActorByImdbID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Actor": %w`, err)
				}

				entity.ID, err = ec.unmarshalNID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
				if err != nil {
					return err
				}
				entity.Name, err = ec.unmarshalNString2string(ctx, rep["name"])
				if err != nil {
					return err
				}
				entity.MovieImdbIds, err = ec.unmarshalNString2ᚕstringᚄ(ctx, rep["movieImdbIds"])
				if err != nil {
					return err
				}
				entity.ImdbID, err = ec.unmarshalNString2string(ctx, rep["imdbId"])
				if err != nil {
					return err
				}
				list[idx[i]] = entity
...
func entityResolverNameForActor(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
		...
		return "findActorByImdbID", nil
	}
	return "", fmt.Errorf("%w for Actor", ErrTypeNotFound)
}
